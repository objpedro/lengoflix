...
export function MovieProvider(props) {
  ...
  const MovieProviderValue: IMovieProvider = {
    ...
    nowPlayingList,
    setNowPlayingList,
    getMoviesUpcoming: async (page: number) => {
      setLoad(true);
    ...
    getMoviesTopRated: async (page: number) => {
      setLoad(true);
      let ret: ProviderResult = null;
    ...
    getMoviesPopular: async (page: number) => {
      setLoad(true);
      let ret: ProviderResult = null;
    ...
    getMoviesNowPlaying: async (page: number) => {
      setLoad(true);
      let ret: ProviderResult = null;
      const requestResult = await tmdbService(page, '/now_playing');
      if (requestResult) {
        setNowPlayingList([...nowPlayingList, ...requestResult]);
        // console.log("Movie Provider Sucesso: ", requestResult);
        ret = {
          ...ret,
          sucesso: true,
